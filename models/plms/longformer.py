...
class Linear(nn.Linear):
    ...
    def forward(self, input: Tensor, p: Optional[torch.Tensor] = None,type: Optional[str] = None) -> Tensor:
...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.query = Linear(config.hidden_size, self.embed_dim)
        self.key = Linear(config.hidden_size, self.embed_dim)
        self.value = Linear(config.hidden_size, self.embed_dim)

        # separate projection layers for tokens with global attention
        self.query_global = Linear(config.hidden_size, self.embed_dim)
        self.key_global = Linear(config.hidden_size, self.embed_dim)
        self.value_global = Linear(config.hidden_size, self.embed_dim)
    ...
    def forward(
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        query_vectors = self.query(hidden_states,p,type)
        key_vectors = self.key(hidden_states,p,type)
        value_vectors = self.value(hidden_states,p,type)
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        global_query_vectors_only_global = self.query_global(global_attn_hidden_states,p,type)
        global_key_vectors = self.key_global(hidden_states,p,type)
        global_value_vectors = self.value_global(hidden_states,p,type)
